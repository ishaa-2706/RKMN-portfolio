...
export default function MagneticHeading({
  ...
  useEffect(() => {
    ...
    // Create GSAP quickTo setters for 60 FPS interpolation
    const setters = chars.map((char) => ({
      ...
      rotate: gsap.quickTo(char, 'rotate', { duration: 0.4, ease: 'power2.out' }),
      scale: gsap.quickTo(char, 'scale', { duration: 0.4, ease: 'power2.out' }),
    }));
    ...
    const handleMouseMove = (e) => {
      ...
      chars.forEach((char, idx) => {
        ...
        if (dist < radius) {
          ...
          setters[idx].x(moveX);
          setters[idx].y(moveY);
          setters[idx].rotate(rotateDeg);
          setters[idx].scale(scaleVal);
        } else {
          setters[idx].x(0);
          setters[idx].y(0);
          setters[idx].rotate(0);
          setters[idx].scale(1);
        }
      });
    };
  ...
  return (
    <Component
      ref={containerRef}
      className={`inline-flex flex-wrap items-center justify-center gap-x-[0.3em] gap-y-[0.1em] cursor-default select-none ${className}`}
    >
